...
class HSICLasso(object):
    def __init__(self):
        ...
        self.X_in = None  # 输入数据
        self.Y_in = None  # 输入标签
        self.X = None     # 处理后的X_in
        self.Xty = None   # 为了计算β得到的中间向量
        self.path = None
        self.beta = None  # β
        self.A = None     # 选出来的特征的index
        self.A_neighbors = None
        self.A_neighbors_score = None
        self.lam = None
        self.featname = None     #数据集中所有特征的名字
    ...
    #  如果有多个输出值，则取出，此时output_list是一个元组，里面存的是输出类别的名字
        else: output_list = ['class']            # 字典参数里没有‘output_list’的话这样赋值
